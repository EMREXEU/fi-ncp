...
### Frontend
You can build frontend separately when needed ex. sourcemaps with `ng build -c development` see package.json scripts for more examples. `angular.json` has configurations for angular app. For develop mode for now you need to modify pom.xml ng build section and remove omit-dev and change prodcution -> development. This should be automated in the future.
